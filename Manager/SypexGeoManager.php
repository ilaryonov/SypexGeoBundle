...
class SypexGeoManager
{
    ...
    public function __construct(FileLocator $fileLocator, $mode, $databasePath)
    {
        ...
        $info = unpack('Cver/Ntime/Ctype/Ccharset/Cb_idx_len/nm_idx_len/nrange/Ndb_items/Cid_len/nmax_region/nmax_city/Nregion_size/Ncity_size/nmax_country/Ncountry_size/npack_size',
            substr($header, 3));
    ...
    public function get_num($ip)
    {
        ...
        if ($blocks['max'] - $blocks['min'] > $this->range) {
            ...
            $part = $this->search_idx($ipn, floor($blocks['min'] / $this->range),
    ...
    {
        $sBaseEncodung = mb_internal_encoding();
        ...
        mb_internal_encoding($sBaseEncodung);
        return $result;
    }
    ...
    public function getData(string $method, string $ip) {
        $sBaseEncodung = mb_internal_encoding();
        mb_internal_encoding('8bit');
        if(method_exists($this, $method)) {
            $result = call_user_func([$this, $method], $ip);
        } else {
            return false;
        }
        mb_internal_encoding($sBaseEncodung);
